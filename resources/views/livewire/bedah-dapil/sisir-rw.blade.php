...
<div style="min-height:100vh;background:#fafafa;">
    <div style="width:100%;margin:0;">
        ...
        <div style="background:white;border:0.5px solid #e5e5e5;border-top:none;border-radius:0 0 14px 14px;overflow:hidden;">
            ...
            <div style="display:grid;grid-template-columns:minmax(0,0.95fr) minmax(340px,1.05fr);gap:12px;padding:0 20px;align-items:start;" class="top-grid">
                <div style="background:white;border:0.5px solid #e5e5e5;border-radius:12px;padding:14px;">
                    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-bottom:10px;">
                        <div>
                            <div style="font-size:11px;color:#fe5000;font-weight:500;letter-spacing:0.8px;text-transform:uppercase;">Heatmap Kunjungan</div>
                            <div style="font-size:14px;color:#1a1a1a;font-weight:500;margin-top:2px;">Frekuensi sisir per RW · {{ $monthLabel }} {{ $yearLabel }}</div>
                        </div>
                        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;font-size:10px;color:#888;">
                            <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#fee2e2;display:inline-block;"></span>0x</span>
                            <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#fed7aa;display:inline-block;"></span>1x</span>
                            <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#bbf7d0;display:inline-block;"></span>2x</span>
                            <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#16a34a;display:inline-block;"></span>3x+</span>
                        </div>
                    </div>

                    <div style="max-height:620px;overflow-y:auto;padding-right:4px;display:grid;gap:12px;">
                        @forelse ($heatmap as $desa)
                            <div style="border:0.5px solid #efefef;border-radius:10px;padding:10px 12px;background:#fcfcfc;">
                                <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-bottom:8px;">
                                    <div style="font-size:12px;font-weight:500;color:#1a1a1a;">{{ $desa['desa'] }}</div>
                                    <div style="font-size:10px;color:#888;">{{ $desa['kecamatan'] }} · {{ $desa['rw_list']->count() }} RW</div>
                                </div>
                                <div style="display:flex;flex-wrap:wrap;gap:6px;">
                                    @foreach ($desa['rw_list'] as $rw)
                                        ...
                                        @endphp
                                        <button
                                            wire:click="openFormForRw('{{ $desa['target_wilayah_id'] }}', '{{ $rw['nomor_rw'] }}')"
                                            type="button"
                                            title="RW {{ $rw['nomor_rw'] }} · {{ $count }}x kegiatan · DPT {{ number_format($rw['dpt']) }} · Est. PKS ~{{ number_format($rw['estimasi_pks']) }}"
                                            style="width:34px;height:34px;border-radius:8px;font-size:11px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;{{ $boxStyle }}"
                                        >
                        ...
                        @empty
                            <div style="border:0.5px dashed #d4d4d8;border-radius:10px;padding:28px 14px;text-align:center;font-size:12px;color:#888;background:#fafafa;">
                                Belum ada data RW untuk heatmap pada filter ini
                            </div>
                        @endforelse
                    </div>
                </div>
                ...
                <div style="background:white;border:0.5px solid #e5e5e5;border-radius:12px;padding:14px;display:grid;gap:12px;">
                    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;flex-wrap:wrap;">
                        ...
                        <div style="font-size:10px;color:#888;">Maksimal 6 item ditampilkan</div>
                    </div>
                    ...
                    <div style="max-height:620px;overflow-y:auto;padding-right:4px;display:grid;gap:8px;">
                        @forelse ($timeline as $kegiatan)
                            ...
                            <div style="border:0.5px solid #ededed;border-radius:10px;background:#fcfcfc;padding:10px 12px;">
                                <div style="display:grid;grid-template-columns:minmax(92px,auto) minmax(0,1fr) auto;gap:12px;align-items:start;">
                                    ...
                                    <button wire:click="editKegiatan('{{ $kegiatan->id }}')" type="button" style="padding:5px 10px;border-radius:6px;border:0.5px solid #d4d4d8;background:white;color:#444;font-size:11px;cursor:pointer;">
                                        Edit
                                    </button>
                                    @if (! $kegiatan->event_id_linked)
                                        <a href="{{ route('events.create', ['from_kegiatan' => $kegiatan->id]) }}" wire:navigate style="display:inline-flex;align-items:center;gap:5px;padding:5px 10px;border-radius:6px;border:0.5px solid #fed7aa;background:#fff7ed;color:#c2410c;font-size:11px;font-weight:500;text-decoration:none;">
                                            <i class="ti ti-calendar-plus" style="font-size:11px;" aria-hidden="true"></i>
                                            Jadikan Event
                                        </a>
                                    @else
                                        <a href="{{ route('events.detail', $kegiatan->event_id_linked) }}" wire:navigate style="display:inline-flex;align-items:center;gap:5px;padding:5px 10px;border-radius:6px;border:0.5px solid #bfdbfe;background:#eff6ff;color:#1d4ed8;font-size:11px;font-weight:500;text-decoration:none;">
                                    ...
                                    @if (filled($kegiatan->catatan) || filled($kegiatan->tindak_lanjut) || filled($kegiatan->tokoh_ditemui))
                                        <a
                                            ...
                                            wire:navigate
                                            style="display:inline-flex;align-items:center;gap:5px;padding:5px 10px;border-radius:6px;border:0.5px solid #bae6fd;background:#ecfeff;color:#0369a1;font-size:11px;font-weight:600;text-decoration:none;">
                                            <i class="ti ti-message-chatbot" style="font-size:11px;" aria-hidden="true"></i>
                                            Jadikan Aspirasi
                                        </a>
                                    @endif
                                    <button wire:click="hapusKegiatan('{{ $kegiatan->id }}')" onclick="return confirm('Yakin ingin menghapus kegiatan ini?')" type="button" style="padding:5px 10px;border-radius:6px;border:0.5px solid #fecaca;background:#fef2f2;color:#dc2626;font-size:11px;cursor:pointer;">
                                        Hapus
                                    </button>
                                </div>
                            </div>
