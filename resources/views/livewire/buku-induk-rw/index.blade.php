<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('buku-induk-rw.download-pdf-sisir-rw', ['tahun' => $selectedTahun]) }}" target="_blank" style="background-color: #ea580c; color: #fff;" class="inline-flex items-center px-4 py-2 border border-transparent rounded-lg font-semibold text-xs uppercase tracking-widest focus:outline-none shadow-sm opacity-90 hover:opacity-100 transition-opacity">
            ...
            <button wire:click="generatePrompt" style="background: linear-gradient(135deg, #FFD700, #D4AF37); color: #fff; border: 1px solid #B8860B;" class="inline-flex items-center px-4 py-2 border rounded-lg font-semibold text-xs uppercase tracking-widest focus:outline-none shadow-md opacity-90 hover:opacity-100 transition-transform transform hover:scale-105 cursor-pointer">
                <svg class="w-4 h-4 mr-2 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                AI Poster Prompt (Top 5 DPC)
            </button>
            <button wire:click="generateDapilPrompt" style="background: linear-gradient(135deg, #2563eb, #1e3a8a); color: #fff; border: 1px solid #1e40af;" class="inline-flex items-center px-4 py-2 border rounded-lg font-semibold text-xs uppercase tracking-widest focus:outline-none shadow-md opacity-90 hover:opacity-100 transition-transform transform hover:scale-105 cursor-pointer">
                <svg class="w-4 h-4 mr-2 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                AI Poster Prompt (DAPIL)
            </button>
        </div>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            ...
            <div class="text-xs text-gray-500 mt-1.5">{{ number_format($summary['total_rt']) }} RT Â· {{ number_format($summary['total_desa']) }} Desa</div>
        </div>
        @php
                    ...
                    @forelse($rws as $rw)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @if($profil && !empty($profil->nama_wilayah))
                                    <div class="text-xs font-semibold text-orange-600 mt-0.5">{{ $profil->nama_wilayah }}</div>
                                @endif
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <a href="{{ route('buku-induk-rw.detail', ['dataRw' => $rw->id, 'action' => 'profil']) }}" wire:navigate class="inline-flex items-center px-3 py-2 border border-blue-200 shadow-sm text-xs font-medium rounded-md text-blue-700 bg-blue-50 hover:bg-blue-100 focus:outline-none transition-colors">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                        </svg>
                                        Profil RW
                                    </a>
                                    <a href="{{ route('buku-induk-rw.detail', ['dataRw' => $rw->id, 'action' => 'struktur']) }}" wire:navigate class="inline-flex items-center px-3 py-2 border border-emerald-200 shadow-sm text-xs font-medium rounded-md text-emerald-700 bg-emerald-50 hover:bg-emerald-100 focus:outline-none transition-colors">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                                        </svg>
                                        Struktur
                                    </a>
                                    <a href="{{ route('buku-induk-rw.detail', $rw->id) }}" wire:navigate class="inline-flex items-center px-3 py-2 border border-transparent text-xs font-medium rounded-md shadow-sm text-white bg-gray-900 hover:bg-gray-800 focus:outline-none transition-colors">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="p-4 space-y-4">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            @if($profil && !empty($profil->nama_wilayah))
                                <div class="text-xs font-semibold text-orange-600 mt-1">{{ $profil->nama_wilayah }}</div>
                            @endif
                    ...
                    <div class="grid grid-cols-2 gap-2 mt-4">
                        ...
                        <a href="{{ route('buku-induk-rw.detail', ['dataRw' => $rw->id, 'action' => 'profil']) }}" wire:navigate class="flex items-center justify-center px-3 py-2.5 border border-blue-200 text-xs font-medium rounded-lg shadow-sm text-blue-700 bg-blue-50 hover:bg-blue-100 focus:outline-none transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                            </svg>
                            Profil RW
                        </a>
                        <a href="{{ route('buku-induk-rw.detail', ['dataRw' => $rw->id, 'action' => 'struktur']) }}" wire:navigate class="flex items-center justify-center px-3 py-2.5 border border-emerald-200 text-xs font-medium rounded-lg shadow-sm text-emerald-700 bg-emerald-50 hover:bg-emerald-100 focus:outline-none transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                            </svg>
                            Struktur
                        </a>
                        <a href="{{ route('buku-induk-rw.detail', $rw->id) }}" wire:navigate class="flex items-center justify-center px-3 py-2.5 border border-transparent text-xs font-medium rounded-lg shadow-sm text-white bg-gray-900 hover:bg-gray-800 focus:outline-none transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
                            </svg>
                            Buka Peta
    ...
    @if($showPromptModal)
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900 bg-opacity-75 transition-opacity" style="z-index: 100;">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-3xl flex flex-col max-h-[90vh]">
            <div class="flex justify-between items-center px-6 py-4 border-b">
                <h3 class="text-lg font-bold text-gray-800">Prompt AI Poster Rekapitulasi</h3>
                <button wire:click="closePromptModal" class="text-gray-400 hover:text-red-500 focus:outline-none transition">
            ...
            <div class="p-6 overflow-y-auto">
                <p class="text-sm text-gray-600 mb-3">
                    Silakan <b>Copy (Salin)</b> teks prompt di bawah ini dan <b>Paste (Tempel)</b> ke dalam ChatGPT (DALL-E) atau Bing Image Creator untuk meng-generate gambar Poster Rekapitulasi Anda. Data di bawah ini diambil otomatis dari database <i>real-time</i> berdasarkan Tahun Target.
                </p>
                <div class="relative">
                    <textarea id="aiPromptTextarea" rows="16" class="w-full text-sm font-mono text-gray-800 bg-gray-50 border border-gray-300 rounded-lg p-4 focus:ring-blue-500 focus:border-blue-500" readonly>{{ $aiPrompt }}</textarea>
                </div>
            </div>

            <div class="flex justify-end px-6 py-4 border-t bg-gray-50 rounded-b-lg gap-3">
                <button wire:click="closePromptModal" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg font-semibold hover:bg-gray-400 transition">Tutup</button>
                <button onclick="copyPromptToClipboard()" class="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition flex items-center shadow-md">
    ...
    <script>
        function copyPromptToClipboard() {
            ...
            try {
                ...
                alert('Prompt berhasil disalin ke clipboard! Silakan paste ke ChatGPT / DALL-E.');
            } catch (err) {}
        }
    </script>
    @endif
