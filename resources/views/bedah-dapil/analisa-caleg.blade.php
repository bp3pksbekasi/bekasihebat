        ...
        <div style="min-height:100vh;background:#fafafa;">
            <div style="width:100%;margin:0;">
                <div style="background:#1a1a1a;color:white;padding:12px 20px;display:flex;align-items:center;justify-content:flex-start;border-radius:14px 14px 0 0;gap:16px;flex-wrap:nowrap;overflow:hidden;">
                    <div style="display:flex;align-items:center;gap:18px;flex-wrap:nowrap;flex:1 1 auto;min-width:0;overflow:hidden;">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;flex-wrap:nowrap;min-width:0;">
                            ...
                            <button id="resetFilterBtn" type="button" style="padding:5px 10px;border:0.5px solid #3f3f46;border-radius:6px;font-size:12px;background:#18181b;color:#f4f4f5;cursor:pointer;white-space:nowrap;">Reset</button>
                        </div>
                    </div>
                ...
                <div style="background:white;border:0.5px solid #e5e5e5;border-top:none;border-radius:0 0 14px 14px;overflow:hidden;">
                    ...
                    <div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:14px;padding:0 20px;align-items:start;" class="double-grid">
                        <div style="background:white;border:0.5px solid #e5e5e5;border-radius:12px;padding:14px;">
                            ...
                            <div id="calegRankingWrap" style="display:grid;gap:6px;"></div>
                        </div>
                        ...
                        <div style="background:white;border:0.5px solid #e5e5e5;border-radius:12px;padding:14px;">
                            ...
                            <div id="pksSebaranWrap" style="display:grid;gap:12px;"></div>
                        </div>
                    </div>

                    <div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:14px;padding:14px 20px 0;align-items:start;" class="double-grid">
                        <div style="background:white;border:0.5px solid #e5e5e5;border-radius:12px;padding:14px;">
                            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px;">
                                <div>
                            ...
                            <div id="headToHeadWrap"></div>
                        </div>

                        <div style="background:white;border:0.5px solid #e5e5e5;border-radius:12px;padding:14px;">
                            <div style="margin-bottom:12px;">
                                <div style="font-size:11px;color:#fe5000;font-weight:500;letter-spacing:0.8px;text-transform:uppercase;">Analisa Gender</div>
                                <div style="font-size:14px;color:#1a1a1a;font-weight:500;margin-top:2px;">Komposisi caleg & efektivitas suara</div>
                            </div>
                            <div id="genderAnalysisWrap"></div>
                        </div>
                    </div>
        ...
        <script>
            ...
            function buildScopeData(dapilKey) {
                if (!state.dataset) {
                    return {
                        ...
                        headToHead: [], totalSuara: 0, totalParties: 0, pksShare: 0, pksRank: 0,
                    };
                }
                ...
                const totalSuara = visibleCaleg.reduce((sum, caleg) => sum + caleg.totalSuara, 0);
                const pksParty = topPartai.find((party) => party.partai === 'PKS');
                const pksCaleg = visibleCaleg.filter((caleg) => caleg.partai === 'PKS').sort((a, b) => b.totalSuara - a.totalSuara);
            ...
            function buildPartySummary(calegList) {
                ...
                calegList.forEach((caleg) => {
                    ...
                    party.totalSuara += caleg.totalSuara;
                    party.calegCount += 1;
            ...
            function buildHeadToHead(targetDapils) {
                ...
                targetDapils.forEach((dapilObj) => {
                    if (!dapilObj) return;
                    dapilObj.villagePartyMap.forEach((village) => {
            ...
            function buildGenderStats(calegList, topPartai) {
                ...
                const totalMaleVotes = maleCaleg.reduce((sum, caleg) => sum + caleg.totalSuara, 0);
                const totalFemaleVotes = femaleCaleg.reduce((sum, caleg) => sum + caleg.totalSuara, 0);
                const pksFemale = calegList.filter((caleg) => caleg.partai === 'PKS' && caleg.gender === 'P').sort((a, b) => b.totalSuara - a.totalSuara);
                const totalPksVotes = calegList.filter((caleg) => caleg.partai === 'PKS').reduce((sum, caleg) => sum + caleg.totalSuara, 0);
                const pksFemaleVotes = pksFemale.reduce((sum, caleg) => sum + caleg.totalSuara, 0);
                const highlightFemale = pksFemale[0];
                const insight = `PKS punya ${formatNumber(pksFemale.length)} caleg perempuan pada scope ini. ${highlightFemale ? `${highlightFemale.nama} meraih suara tertinggi (${formatNumber(highlightFemale.totalSuara)}).` : 'Belum ada caleg perempuan PKS pada scope ini.'} Caleg perempuan PKS meraup ${formatPercent(totalPksVotes ? (pksFemaleVotes / totalPksVotes) : 0)} dari total suara PKS.`;
                return {
                    perParty,
            ...
            function renderSummaryCards(scopeData) {
                ...
                const totalPksVotes = totalPksCaleg.reduce((sum, caleg) => sum + caleg.totalSuara, 0);
                    ...
                    return `
                        <button type="button" data-open-caleg="${escapeHtml(caleg.key)}" style="display:flex;align-items:center;gap:8px;padding:8px;background:${background};border-radius:7px;border:0.5px solid ${border};margin-bottom:0;cursor:pointer;text-align:left;">
                            ...
                            <div style="text-align:right;">
                                <div style="font-size:13px;font-weight:500;color:${isPks ? '#fe5000' : '#1a1a1a'};">${formatNumber(caleg.totalSuara)}</div>
                                ${suffix}
                            </div>
                        </button>`;
                }).join('');
                        ...
                        <div style="${index > 0 ? 'border-top:0.5px solid #f0f0f0;padding-top:10px;' : ''}">
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:8px;">
                                <button type="button" data-open-caleg="${escapeHtml(caleg.key)}" style="background:none;border:none;padding:0;cursor:pointer;text-align:left;">
                                    ...
                                    <span style="font-size:10px;color:#666;margin-left:6px;">No. ${escapeHtml(String(caleg.nomorUrut))} · ${escapeHtml(formatGender(caleg.gender))} · ${formatNumber(caleg.totalSuara)} suara</span>
                                </button>
            ...
            function renderHeadToHead(scopeData) {
                ...
                dom.headToHeadWrap.innerHTML = `
                    <div style="overflow:auto;border:0.5px solid #e5e5e5;border-radius:10px;">
                ...
                const body = currentRows.map((caleg, index) => {
                    ...
                    return `
                        <tr style="background:${background};border-bottom:0.5px solid #eee;">
                            ...
                            <td style="padding:9px 10px;font-size:12px;text-align:right;font-weight:500;color:${isPks ? '#fe5000' : '#1a1a1a'};">${formatNumber(caleg.totalSuara)}</td>
                            <td style="padding:9px 10px;font-size:12px;color:#444;text-align:right;">${formatPercent(caleg.partyTotalSuara ? (caleg.totalSuara / caleg.partyTotalSuara) : 0)}</td>
                    ...
                    <div style="margin-top:16px;">
                        ...
                        <div style="overflow:auto;border:0.5px solid #e5e5e5;border-radius:10px;margin-top:8px;">
                            <table style="width:100%;border-collapse:collapse;background:white;">
                                ...
                                <tbody>${topVillages.map((row) => `<tr><td style="padding:8px 10px;border-bottom:0.5px solid #eee;font-size:12px;color:#1a1a1a;">${escapeHtml(row.desa)}</td><td style="padding:8px 10px;border-bottom:0.5px solid #eee;font-size:12px;color:#444;">${escapeHtml(row.kecamatan)}</td><td style="padding:8px 10px;border-bottom:0.5px solid #eee;font-size:12px;text-align:right;font-weight:500;">${formatNumber(row.suara)}</td><td style="padding:8px 10px;border-bottom:0.5px solid #eee;font-size:12px;text-align:right;color:#666;">${formatPercent(caleg.totalSuara ? (row.suara / caleg.totalSuara) : 0)}</td></tr>`).join('')}</tbody>
                            </table>
                        </div>
                    </div>
            ...
            function exportVisibleCsv() {
                ...
                const csvRows = [
                    ...
                    ...rows.map((caleg, index) => [
                        ...
                        caleg.gender,
                        caleg.totalSuara,
                        (caleg.partyTotalSuara ? ((caleg.totalSuara / caleg.partyTotalSuara) * 100).toFixed(2) : '0.00'),
